...
func (p *Provider) Create(ctx context.Context, nodeClaim *karpenterv1.NodeClaim) (*Instance, error) {
	...
	if len(apName) > 11 {
		...
		return nil, cloudprovider.NewInvalidParameterError(fmt.Errorf("the length agentpool name should be less than 11, got %d (%s)", len(apName), apName))
	}
	...
	err := retry.OnError(retry.DefaultBackoff, func(err error) bool {
		...
		if err != nil {
			...
			case strings.Contains(err.Error(), "Operation is not allowed because there's an in progress create node pool operation"):
				...
				ap = &apObj
				return nil
			case strings.Contains(err.Error(), "all requested instance types were unavailable during launch") && len(instanceTypes) == 1:
				return cloudprovider.NewInsufficientCapacityError(err)
			case strings.Contains(err.Error(), "InvalidParameter") && len(instanceTypes) == 1:
				return cloudprovider.NewInvalidParameterError(err)
			case strings.Contains(err.Error(), "is not allowed in your subscription in location") && len(instanceTypes) == 1:
				return cloudprovider.NewLocationRestrictionError(err)
			default:

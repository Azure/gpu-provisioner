...
// InvalidParameterError is an error type returned by CloudProviders when a launch fails due to invalid parameter of agent pool.
type InvalidParameterError struct {
	error
}

func NewInvalidParameterError(err error) *InvalidParameterError {
...
func (e *InvalidParameterError) Error() string {
	return fmt.Sprintf("invalid parameter, %s", e.error)
}
...
// LocationRestrictionError is an error type returned by CloudProviders when a launch fails due to not allowed in your subscription in location.
type LocationRestrictionError struct {
	error
}

func NewLocationRestrictionError(err error) *LocationRestrictionError {
	return &LocationRestrictionError{
		error: err,
	}
}

func (e *LocationRestrictionError) Error() string {
	return fmt.Sprintf("subscription location restriction, %s", e.error)
}

func IsLocationRestrictionError(err error) bool {
	if err == nil {
		return false
	}
	var ipErr *LocationRestrictionError
	return errors.As(err, &ipErr)
}

...
func (l *Launch) launchNodeClaim(ctx context.Context, nodeClaim *v1.NodeClaim) (*v1.NodeClaim, error) {
	...
	if err != nil {
		...
		case cloudprovider.IsInsufficientCapacityError(err) ||
			cloudprovider.IsLocationRestrictionError(err) ||
			cloudprovider.IsInvalidParameterError(err):
			// if launch nodeClaim failed due to these error reason, there is no need to retry
			// launch nodeClaim again. we only need to publish a event to notify user and delete
			// nodeClaim for eliminating cloudprovider resouce usage.
			// l.recorder.Publish(InsufficientCapacityErrorEvent(nodeClaim, err))
			event, reasonLabel := NodeClaimErrorEventAndReasonLabel(nodeClaim, err)
			l.recorder.Publish(event)
			...
			metrics.NodeClaimsDisruptedTotal.With(prometheus.Labels{
				...
				metrics.ReasonLabel:       reasonLabel,
